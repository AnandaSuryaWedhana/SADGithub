...
<body>
    ...
    <form action="" method="post">
      ...
      <input class="form-control" value="{{ $data[0]->NAMA_PEMBELI }}" type="text" placeholder="Nama Customer" aria-label="default input example" required/>
      <div class="col-12" style="padding-top: 10px">
        <label for="inputAddress" class="form-label">Alamat : </label>
        <input type="text" class="form-control" id="inputAddress" placeholder="1234 Main St" value="{{ $data[0]->ALAMAT_PEMBELI }}" required/>
      </div>
      <div class="seperatediv" style="margin-top: 10px; display:flex">
        <div class="col-md-6" style="width: 70%">
          <label for="inputAddress" class="form-label">Nomor Telpon :</label>
          <input type="tel" class="form-control" pattern="[0-9]{3}-[0-9]{2}-[0-9]{3}" value="{{ $data[0]->TELPHONE_PEMBELI }}" required />
        </div>
        ...
        <div class="mb-3" style="width: 25%">
          ...
          <input type="number" class="form-control" id="exampleFormControlInput1" value="{{ $data[0]->JUMLAHPRODUK_TRANSAKSI }}" required/>
        </div>
      </div>
